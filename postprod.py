...
input_year = raw_input("Jahr? (z.B. 2016, fuer heutiges Datum einfach <ENTER>) > ")
day = ''
if input_year == "":
    ...
    while day == '':
        input_month = raw_input("Monat? (z.B. 04) > ")
...
broadcast = ""
while broadcast == "":
    input_broadcast = raw_input("Sendung? " + str(broadcasts.keys()) + " > ")
    try:
...
input_number = raw_input("Folgenzahl? (z.B. 197) > ")
...
print("\033[1m# FLAC-Datei umgewandelt \n# von '%s'\n# zu  '%s.opus'\033[0m" % (flacfile, newfilename ))
...
for line in pre_psc_content:
    title = re.findall(r'(<=\d\d:\d\d\:\d\d\.000\ )*', line)
